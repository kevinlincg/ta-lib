...
it has an output for, in the input series' coordinates: `TA_<NAME>_OutRange` in
C, `out_range()`, `outRange()`, `OutRange`. Which calls move it is
`docs/error-handling-spec.md` §2.4's business.

**The advance call counts a bar the handle was not fed** — `TA_<NAME>_Advance`,
`advance()`, `advance()`, `Advance()`. All four backends emit it per handle
class, as they do the range accessor (#387). It moves the count by one and nothing else, so
