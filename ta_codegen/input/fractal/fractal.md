...
- `optInLeftBars` — Bars that must be strictly exceeded on the candidate's left
- `optInRightBars` — Bars that must be strictly exceeded on the candidate's right, and the delay before the verdict is reported
