 ...
 */
                        ...
                        double outReal[] )
   {
      ...
      requireLength("ADR", "inLow", inLow, guardInLen);
...
/**** Streaming API *****/
   ...
   private RetCode adrOpenImpl( AdrStream sp, double inHigh[], double inLow[], int startIdx, int optInTimePeriod, MInteger outBegIdx, MInteger outNBElement, double outReal[], int outStride )
   {
      double periodTotal = 0;
      double tempReal = 0;
      int i = 0;
      ...
      for( int fillJ = historyLen - cap_trailingIdx; fillJ < historyLen; fillJ++ ) {
         capRing_trailingIdx_derived[fillJ - (historyLen - cap_trailingIdx)] = (double)(inHigh[fillJ] - inLow[fillJ]);
      }
   ...
   /* adrOpenAndFill anchored at startIdx — the composed-open fusion seam. */
   AdrStream adrOpenAndFillInternal( double inHigh[], double inLow[], int startIdx, int optInTimePeriod, MInteger outBegIdx, MInteger outNBElement, double outReal[] )
   {
      AdrStream sp = new AdrStream(this);
      RetCode retCode = adrOpenImpl(sp, inHigh, inLow, startIdx, optInTimePeriod, outBegIdx, outNBElement, outReal, 1);
   ...
   AdrStream adrOpenInternal( double inHigh[], double inLow[], int startIdx, int optInTimePeriod )
   {
      ...
      RetCode retCode = adrOpenImpl(sp, inHigh, inLow, startIdx, optInTimePeriod, outBegIdx, outNBElement, sink_outReal, 0);
      sp.outRangeBegIdx = outBegIdx.value;
      sp.outRangeCount = outNBElement.value;
      if( retCode == RetCode.Success ) {
         return sp;
    ...
    */
   public AdrStream adrOpen( double inHigh[], double inLow[], int optInTimePeriod )
   {
      requireArgument("ADR open", "inHigh", inHigh);
      requireHistory("ADR open", inHigh.length);
      requireArgument("ADR open", "inLow", inLow);
      requireHistoryLength("ADR open", "inLow", inLow.length, inHigh.length);
      return adrOpenInternal(inHigh, inLow, 0, optInTimePeriod);
   }
   /**
    ...
    * {@link AdrStream#outRange()}.
    */
   public AdrStream adrOpenAndFill( double inHigh[], double inLow[], int optInTimePeriod, double outReal[] )
   {
      ...
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
      return adrOpenAndFillInternal(inHigh, inLow, 0, optInTimePeriod, outBegIdx, outNBElement, outReal);
   }
