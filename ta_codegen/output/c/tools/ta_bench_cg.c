...
static void bench_all(const char *filter, int iters) {
    ...
    if( func_matches(filter, "CMOU") ) {
        ...
        for( int pass = 0; pass < 3; pass++ ) {
            ...
            long long t0 = get_nanotime();
            for( int it = 0; it < iters; it++ ) {
                TA_CMOU(0, g_nPoints - 1, g_close, 14, &outBegIdx, &outNBElement, g_outBuf0);
            }
            long long elapsed = get_nanotime() - t0;
            if( !best || elapsed < best ) best = elapsed;
            g_sink += outNBElement;
    ...
    if( func_matches(filter, "RMA") ) {
        ...
        for( int pass = 0; pass < 3; pass++ ) {
            ...
            long long t0 = get_nanotime();
            for( int it = 0; it < iters; it++ ) {
                TA_RMA(0, g_nPoints - 1, g_close, 30, &outBegIdx, &outNBElement, g_outBuf0);
            }
            long long elapsed = get_nanotime() - t0;
            if( !best || elapsed < best ) best = elapsed;
            g_sink += outNBElement;
    ...
    if( func_matches(filter, "SMI") ) {
        ...
        for( int pass = 0; pass < 3; pass++ ) {
            ...
            long long t0 = get_nanotime();
            for( int it = 0; it < iters; it++ ) {
                TA_SMI(0, g_nPoints - 1, g_high, g_low, g_close, 13, 2, 25, 9, &outBegIdx, &outNBElement, g_outBuf0, g_outBuf1);
            }
            long long elapsed = get_nanotime() - t0;
            if( !best || elapsed < best ) best = elapsed;
            g_sink += outNBElement;
    ...
    if( func_matches(filter, "VHF") ) {
        ...
        for( int pass = 0; pass < 3; pass++ ) {
            ...
            long long t0 = get_nanotime();
            for( int it = 0; it < iters; it++ ) {
                TA_VHF(0, g_nPoints - 1, g_close, 28, &outBegIdx, &outNBElement, g_outBuf0);
            }
            long long elapsed = get_nanotime() - t0;
            if( !best || elapsed < best ) best = elapsed;
            g_sink += outNBElement;
    ...
    if( func_matches(filter, "VWAP") ) {
        ...
        for( int pass = 0; pass < 3; pass++ ) {
            ...
            long long t0 = get_nanotime();
            for( int it = 0; it < iters; it++ ) {
                TA_VWAP(0, g_nPoints - 1, g_high, g_low, g_close, g_volume, &outBegIdx, &outNBElement, g_outBuf0);
            }
            long long elapsed = get_nanotime() - t0;
            if( !best || elapsed < best ) best = elapsed;
            g_sink += outNBElement;
