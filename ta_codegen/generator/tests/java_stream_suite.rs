...
/// the peek frame's, and a second on any of them would be a commit path that
...
#[test]
fn the_composed_sub_handle_sinks_are_exactly_the_costed_three() {
    ...
    let expected: BTreeMap<String, usize> = [
        ("kdj".to_string(), 1usize),
        ("ma".to_string(), 1usize),
        ("stochrsi".to_string(), 1usize),
    ]
    .into_iter()
    .collect();
    ...
    assert_eq!(
        sites, expected,
        "the composed sub-handle sink sites moved (one peek frame on each of the three \
